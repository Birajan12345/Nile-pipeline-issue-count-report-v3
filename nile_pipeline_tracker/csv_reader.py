...
def read_csv(filepath: str) -> tuple[list[dict], dict]:
    ...
    with open(filepath, newline="", encoding="latin-1") as f:
        ...
        raw_rows = []
        for row in reader:
            cleaned = {field: normalize_text(row.get(col, "")) if col else "" for field, col in {
                field: detect_column(headers, aliases) for field, aliases in COLUMN_ALIASES.items()
            }.items()}
            raw_rows.append(cleaned)

    col_map = {field: detect_column(headers, aliases) for field, aliases in COLUMN_ALIASES.items()}
...
def _extract_html_issue_rows(soup) -> list[dict]:
    ...
    for header_table in soup.select("table.tableBorder"):
        ...
        anchor = title.find("a", class_="issue-link")
        issue_key = normalize_text(anchor.get_text(strip=True)) if anchor else ""
        summary = normalize_text(anchor.get_text(strip=True)) if anchor else normalize_text(title.get_text(" ", strip=True))
        if issue_key and summary.startswith(issue_key):
            summary = summary[len(issue_key) :].strip(" -\u00A0")

        subtext = title.find("span", class_="subText")
        created, updated, resolved = _parse_subtext(subtext.get_text(" ", strip=True)) if subtext else ("", "", "")
        ...
        for node in block:
            ...
            issue_text.append(node.get_text(" ", strip=True))
...
def read_html(filepath: str) -> tuple[list[dict], dict]:
    ...
    try:
        ...
        print("\n  ERROR: BeautifulSoup and lxml are required to parse HTML input.")
        print("  Install them with: pip install beautifulsoup4 lxml")
        sys.exit(1)
...
def parse_rows(raw_rows: list, col_map: dict | None = None) -> list[dict]:
    rows = []
    for r in raw_rows:
        def get(field: str) -> str:
            if col_map:
                col = col_map.get(field)
                return normalize_text(r.get(col, "")) if col else ""
        ...
        new_categories = get("New Categories")
        assignee = get("assignee") or get("Assign")
        ...
        rows.append({
            ...
            "Custom field (Assignment Group)": get("assignment_group"),
            "Assignee": assignee or "Unassigned",
            "Reporter": get("reporter"),
            "Resolution": resolution,
            "Custom field (Comments)": comments,
            "Inward issue link (Cloners)": get("Inward issue link (Cloners)"),
            "Outward issue link (Cloners)": get("Outward issue link (Cloners)"),
            "Inward issue link (Relates)": get("Inward issue link (Relates)"),
            "Category": category,
